...
# Initialize logger
logger = Logger(program_name='benchmark_magnitude_strategies', level=LogLevel.INFO)
...
def compare_strategies(model, sparsity, num_trials=3):
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"Benchmarking Magnitude Strategies")
    logger.info(f"{'='*70}")
    ...
    logger.info("Testing CURRENT MagnitudePruning...")
    ...
    logger.info("Testing OPTIMIZED MagnitudePruningOptimized...")
...
if __name__ == "__main__":
    ...
    if avg_speedup > 3.0 and avg_agreement > 0.995:
        ...
        logger.info("   Ready to replace current magnitude implementation.")
